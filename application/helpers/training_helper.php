...
function gettrNumber($projNameThreeLetters, $areaShortTwoLetters, $blockFirstLetter, $ProjectId)
{

    $ci = &get_instance();

    $ci->db->select('MAX(tr_auto_id) as counts');

    $result = $ci->db->get(TR_FLOW_SEE)->row();
    $ci->db->where(['tr_auto_id' => $ProjectId]);
    $maxid = $result->counts;

    $maxid = $maxid + 1;
    $returnId = str_pad($maxid, 4, 0, STR_PAD_LEFT);
    return $projNameThreeLetters . '-' . 'TRAINING' . '-' . $areaShortTwoLetters . $blockFirstLetter . '-' . $returnId;
}
...
function sendObsNotification($obs_id)
{
    ...
    $option['join'] = [
        ...
        LOGIN . ' AS LD' => ['LD.LOGIN_ID = ' . TR_FLOW_SEE . '.obs_reporter_id', 'LEFT'],
        EMPL . ' AS Reporter' => ['Reporter.EMP_AUTO_ID = LD.USER_REF_ID', 'LEFT'],


        LOGIN . ' AS LD1' => ['LD1.LOGIN_ID = ' . TR_FLOW_SEE . '.obs_supervisor_id', 'LEFT'],
        EMPL . ' AS supervisor' => ['supervisor.EMP_AUTO_ID = LD1.USER_REF_ID', 'LEFT'],

        LOGIN . ' AS LD2' => ['LD2.LOGIN_ID = ' . TR_FLOW_SEE . '.obs_assigner_id', 'LEFT'],
        EMPL . ' AS Hod' => ['Hod.EMP_AUTO_ID = LD2.USER_REF_ID', 'LEFT'],

        LOGIN . ' AS LD4' => ['LD4.LOGIN_ID = ' . TR_FLOW_SEE . '.obs_hsse_es_id', 'LEFT'],
        EMPL . ' AS Hsse_es' => ['Hsse_es.EMP_AUTO_ID = LD4.USER_REF_ID', 'LEFT'],

        LOGIN . ' AS LD3' => ['LD3.LOGIN_ID = ' . TR_FLOW_SEE . '.obs_hsse_id', 'LEFT'],
    ...
    $atardetails = $ci->common_model->getAlldata(TR_FLOW_SEE, [
        '*',
