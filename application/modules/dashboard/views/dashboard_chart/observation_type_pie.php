...
<div id="obs_status">
  <div class="card-header" style="display:none;">
    <h3 class="card-title">OBSERVATION REPORT - BASED ON RISK RATING</h3>
  </div>
...
<script>
  ...
  var risk_color_code = <?php echo json_encode($risk_color_code); ?>;
  if (Array.isArray(obs_statusData) && obs_statusData.length === 0) {
        ...
        events: {
          dataPointSelection: function(event, chartContext, config) {
            ...
            url += '?Month=' + encodeURIComponent(selectedMonth) +
              '&risk_id=' + encodeURIComponent(risk_id) +
    ...
    var obs_statuschart = new ApexCharts($("#obs_status")[0], options);
    obs_statuschart.render();
  }
  $(document).ready(function() {
    $("#obsriskreport").off("click").on("click", function() {
      if (Array.isArray(obs_statusData) && obs_statusData.length === 0) {
        ...
        obs_statuschart.dataURI().then(({
          ...
          image.onload = function() {
            ...
            ctx.font = '20px Arial';
            var headerText = 'OBSERVATION REPORT - BASED ON RISK RATING';
            ...
            newCanvas.toBlob(function(blob) {
              ...
              link.download = 'observation_risk_rating_report.png'; // Change the file name here
              link.click();
            });
          };
          image.src = imgURI;
