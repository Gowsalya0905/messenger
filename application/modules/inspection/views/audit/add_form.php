...
if ($isatar) {
    $formTitle = 'Edit Audit Atar Inspection';
    $back = 'inspection/audit/audit_atar_list';
} else {
    $formTitle = ($editId > 0) ? 'Edit Inspection' : 'Add Inspection';
    $back = 'inspection/audit/inspection_list';
}
                    ...
                    <div class="col-md-12">
                        ...
                        <div class="card">
                            ...
                            <div class="card-body">
                                ...
                                ?>
                                <?php echo form_open_multipart('inspection/audit/save_inspection/' . encryptval($getProid),  'class="form-horizontal" id="company-profile" novalidate'); ?>
                                <input type="hidden" name="audit_ins_id" id="audit_ins_id" value="<?php echo !empty($edit_id) ? $edit_id : '' ?>">
...
<script type="text/javascript">
    $(document).ready(function() {
        ...
        function auditizeValidation(isDraft) {
            // Destroy any existing validation
            $("#company-profile").validate().destroy();
            // Reauditize validation with appropriate settings
    ...
    $(document).ready(function() {
        ...
        function generateDropdown(slNo, editStatus) {
            const dropauditstatus = <?php echo json_encode($dropauditstatus); ?>;

            let dropdown = `<select id="dropdown_${slNo}" name="item[outcome][${slNo}]" class="form-control select2 audit_status">`;

            for (const [value, label] of Object.entries(dropauditstatus)) {
        ...
        function fetchCategoryDetails(categoryId) {
            ...
            $.ajax({
                url: "<?php echo BASE_URL() . 'inspection/audit/fetchCategoryDetails'; ?>",
                type: "POST",
                data: {
        ...
        $(document).on('click', '.delete-btn', function() {
            var url = '<?php echo BASE_URL("inspection/audit/delete_evidence"); ?>';
