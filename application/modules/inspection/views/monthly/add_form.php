...
if ($isatar) {
    $formTitle = 'Edit Monthly Atar Inspection';
    $back = 'inspection/monthly/monthly_atar_list';
} else {
    $formTitle = ($editId > 0) ? 'Edit Inspection' : 'Add Inspection';
    $back = 'inspection/monthly/inspection_list';
}
                    ...
                    <div class="col-md-12">
                        ...
                        <div class="card">
                            ...
                            <div class="card-body">
                                ...
                                ?>
                                <?php echo form_open_multipart('inspection/monthly/save_inspection/' . encryptval($getProid),  'class="form-horizontal" id="company-profile" novalidate'); ?>
                                <input type="hidden" name="monthly_ins_id" id="monthly_ins_id" value="<?php echo !empty($edit_id) ? $edit_id : '' ?>">
                                        ...
                                        <table class="table table-bordered">
                                            <thead class="thead-light">
                                                <tr>
                                                    ...
                                                    <th><i class="fa fa-times text-danger" aria-hidden="true"></i></th>
                                                    <th>Evidence</th>
...
<script type="text/javascript">
    $(document).ready(function() {
        ...
        function monthlyizeValidation(isDraft) {
            // Destroy any existing validation
            $("#company-profile").validate().destroy();
            // Remonthlyize validation with appropriate settings
    ...
    $(document).ready(function() {
        ...
        function generateDropdown(slNo, editStatus) {
            const dropmonthlystatus = <?php echo json_encode($dropmonthlystatus); ?>;

            let dropdown = `<select id="dropdown_${slNo}" name="item[outcome][${slNo}]" class="form-control select2 monthly_status">`;

            for (const [value, label] of Object.entries(dropmonthlystatus)) {
        ...
        function fetchCategoryDetails(categoryId) {
            ...
            $.ajax({
                url: "<?php echo BASE_URL() . 'inspection/monthly/fetchCategoryDetails'; ?>",
                type: "POST",
                data: {
                        ...
                        $.each(response.subcategoriesdata, function(subcatId, subcategory) {
                            ...
                            $.each(subcategory.data, function(key, data) {
                                ...
                                subcategoryTable.append(
                                    ...
                                    '<div class="error-message" style="color: red; display: none;"> Notes / Comments Required.</div>' +
        ...
        $(document).on('click', '.delete-btn', function() {
            var url = '<?php echo BASE_URL("inspection/monthly/delete_evidence"); ?>';
