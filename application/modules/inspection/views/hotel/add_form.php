...
if ($isatar) {
    $formTitle = 'Edit Hotel Atar Inspection';
    $back = 'inspection/hotel/hotel_atar_list';
} else {
    $formTitle = ($editId > 0) ? 'Edit Inspection' : 'Add Inspection';
    $back = 'inspection/hotel/inspection_list';
}
                    ...
                    <div class="col-md-12">
                        ...
                        <div class="card">
                            ...
                            <div class="card-body">
                                ...
                                ?>
                                <?php echo form_open_multipart('inspection/hotel/save_inspection/' . encryptval($getProid),  'class="form-horizontal" id="company-profile" novalidate'); ?>
                                <input type="hidden" name="hotel_ins_id" id="hotel_ins_id" value="<?php echo !empty($edit_id) ? $edit_id : '' ?>">
...
<script type="text/javascript">
    $(document).ready(function() {
        ...
        function hotelizeValidation(isDraft) {
            // Destroy any existing validation
            $("#company-profile").validate().destroy();
            // Rehotelize validation with appropriate settings
    ...
    $(document).ready(function() {
        ...
        function generateDropdown(slNo, editStatus) {
            const drophotelstatus = <?php echo json_encode($drophotelstatus); ?>;

            let dropdown = `<select id="dropdown_${slNo}" name="item[outcome][${slNo}]" class="form-control select2 hotel_status">`;

            for (const [value, label] of Object.entries(drophotelstatus)) {
        ...
        function fetchCategoryDetails(categoryId) {
            ...
            $.ajax({
                url: "<?php echo BASE_URL() . 'inspection/hotel/fetchCategoryDetails'; ?>",
                type: "POST",
                data: {
        ...
        $(document).on('click', '.delete-btn', function() {
            var url = '<?php echo BASE_URL("inspection/hotel/delete_evidence"); ?>';
