...
if ($isatar) {
    $formTitle = 'Edit Workcamp Atar Inspection';
    $back = 'inspection/workcamp/workcamp_atar_list';
} else {
    $formTitle = ($editId > 0) ? 'Edit Inspection' : 'Add Inspection';
    $back = 'inspection/workcamp/inspection_list';
}
                    ...
                    <div class="col-md-12">
                        ...
                        <div class="card">
                            ...
                            <div class="card-body">
                                ...
                                ?>
                                <?php echo form_open_multipart('inspection/workcamp/save_inspection/' . encryptval($getProid),  'class="form-horizontal" id="company-profile" novalidate'); ?>
                                <input type="hidden" name="workcamp_ins_id" id="workcamp_ins_id" value="<?php echo !empty($edit_id) ? $edit_id : '' ?>">
                                    ...
                                    <div class="row m-t-10" id="subcategory_div" style="display: none">
                                        <table class="table table-bordered">
                                            <thead class="thead-light">
                                                <tr>
                                                    <th>Sl. No.</th>
                                                    <th style="width: 20%;">Items to be checked</th>
                                                    <th style="width: 20%;">Yes/No/NA</th>
...
<script type="text/javascript">
    $(document).ready(function() {
        ...
        function workcampizeValidation(isDraft) {
            // Destroy any existing validation
            $("#company-profile").validate().destroy();
            // Reworkcampize validation with appropriate settings
    ...
    $(document).ready(function() {
        ...
        function generateDropdown(slNo, editStatus) {
            const dropworkcampstatus = <?php echo json_encode($dropworkcampstatus); ?>;

            let dropdown = `<select id="dropdown_${slNo}" name="item[outcome][${slNo}]" class="form-control select2 workcamp_status">`;

            for (const [value, label] of Object.entries(dropworkcampstatus)) {
        ...
        $(document).on('change', '.workcamp_status', function() {
            const itemOutcome = $(this).val();
            const slNo = $(this).data('slno');
            const $row = $(this).closest('tr');

            const updatedScoreDropdown = generateScoreDropdown(slNo, '', itemOutcome);

            $row.find('td:eq(3)').html(updatedScoreDropdown);
            $row.find('.workcamp_score').select2();
        });



        function generateScoreDropdown(slNo, editStatus, itemOutcome) {
            const dropworkcampscore =
                itemOutcome == '0' ?
                <?php echo json_encode($dropworkcampscore); ?> :
                <?php echo json_encode($dropworkcampscore1); ?>;

            let dropdown = `<select id="dropdownscore_${slNo}" name="item[score][${slNo}]" class="form-control select2 workcamp_score">`;

            for (const [value, label] of Object.entries(dropworkcampscore)) {
                const selected = value == editStatus ? 'selected' : '';
                dropdown += `<option value="${value}" ${selected}>${label}</option>`;
            }
        ...
        function fetchCategoryDetails(categoryId) {
            ...
            $.ajax({
                url: "<?php echo BASE_URL() . 'inspection/workcamp/fetchCategoryDetails'; ?>",
                type: "POST",
                data: {
                        ...
                        $.each(response.subcategoriesdata, function(subcatId, subcategory) {
                            ...
                            $.each(subcategory.data, function(key, data) {
                                ...
                                subcategoryTable.append(
                                    ...
                                    '<td >' + generateDropdown(slNo, itemOutcome) + '</td>' +
                                    '<td >' + generateScoreDropdown(slNo, itemScore, itemOutcome) + '</td>' +
                                    ...
                                    '</td>' +
                                    '<td><textarea class="form-control notes-field" name="item[notes][' + slNo + ']" placeholder="Enter Description" >' + itemActionReq + '</textarea>' +
                                    '<div class="error-message" style="color: red; display: none;"> Description required.</div>' +
                                    '</td>' +
        ...
        $(document).on('click', '.delete-btn', function() {
            var url = '<?php echo BASE_URL("inspection/workcamp/delete_evidence"); ?>';
