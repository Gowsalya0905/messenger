...
if ($isatar) {
    $formTitle = 'Edit Weekly Atar Inspection';
    $back = 'inspection/weekly/weekly_atar_list';
} else {
    $formTitle = ($editId > 0) ? 'Edit Inspection' : 'Add Inspection';
    $back = 'inspection/weekly/inspection_list';
}
                    ...
                    <div class="col-md-12">
                        ...
                        <div class="card">
                            ...
                            <div class="card-body">
                                ...
                                ?>
                                <?php echo form_open_multipart('inspection/weekly/save_inspection/' . encryptval($getProid),  'class="form-horizontal" id="company-profile" novalidate'); ?>
                                <input type="hidden" name="weekly_ins_id" id="weekly_ins_id" value="<?php echo !empty($edit_id) ? $edit_id : '' ?>">
...
<script type="text/javascript">
    $(document).ready(function() {
        ...
        function weeklyizeValidation(isDraft) {
            // Destroy any existing validation
            $("#company-profile").validate().destroy();
            // Reweeklyize validation with appropriate settings
    ...
    $(document).ready(function() {
        ...
        function generateDropdown(slNo, editStatus) {
            const dropweeklystatus = <?php echo json_encode($dropweeklystatus); ?>;

            let dropdown = `<select id="dropdown_${slNo}" name="item[outcome][${slNo}]" class="form-control select2 weekly_status">`;

            for (const [value, label] of Object.entries(dropweeklystatus)) {
        ...
        function fetchCategoryDetails(categoryId) {
            ...
            $.ajax({
                url: "<?php echo BASE_URL() . 'inspection/weekly/fetchCategoryDetails'; ?>",
                type: "POST",
                data: {
        ...
        $(document).on('click', '.delete-btn', function() {
            var url = '<?php echo BASE_URL("inspection/weekly/delete_evidence"); ?>';
