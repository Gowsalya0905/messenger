...
class Hotel extends CI_Controller
{
    ...
    public function __construct()
    {
        parent::__construct();
        isLogin();
        $this->load->model('inspection/hotel_model', 'hotel');
    ...
    public function inspection_list()
    {
        ...
        $getAllinscat = $this->common_model->getAlldata(INSP_HOTEL_CAT, ['*'], $inscatOptn);
        $dropinscat = customFormDropDown($getAllinscat, 'id', 'category', 'Select Category');



        $data = [
            'pageTitle' => 'Hotel Inspection',
            'view_file' => 'inspection/hotel/list_form',
            'site_title' => 'Hotel Inspection',
            'current_menu' => 'List',
            ...
            'ajaxurl' => 'inspection/hotel/listInspection?Status=' . $Status . '&company_id=' . $company_id . '&area_id=' . $area_id . '&building_id=' . $building_id  . '&cat_id=' . $cat_id . '&project_id='  . $project_id . '&Department=' . $department_id . '&Start_Date=' . $Start_Date . '&End_Date=' . $End_Date,
        ];
    ...
    public function listInspection()
    {
        ...
        $table = INSP_HOTEL_FLOW_SEE . ' as h';
        $column_order = array(
            null,
            'ins_id',
        ...
        $options['join'][INSP_HOTEL_CAT . ' as cat'] = ['cat.id = h.ins_cat_id', 'left'];
        ...
        if (isset($listUsee) && !empty($listUsee)) {
            foreach ($listUsee as $ltKey => $ltVal) {
                ...
                if (((in_array($user_type, $insPermission['view_supadmin'])) || (in_array($user_type, $insPermission['view_ad']) || $userid == $ins_reporter_id)) && ($stat == 1 || $stat == 0) && ($is_edit == 0)) {
                    $action .= " " . anchor('inspection/hotel/add_inspection/' . $id, '<i class="fa fa-edit"></i>', array('class' => '', 'title' => 'Edit'));
                }
                ...
                $action .= " " . anchor('inspection/hotel/view_form/' . $id, '<i class="fa fa-eye"></i>', array('title' => 'view'));
                $action .= " " . anchor('inspection/hotel/pdf/' . $id, '<i class="fas fa-file-pdf" aria-hidden="true"></i>', array('class' => '', 'title' => 'PDF', 'target' => '_blank'));
    ...
    public function add_inspection($id = '')
    {
        ...
        global $owner_list, $owner_engineer_list, $EPC_list, $hotel_status;
        $current_role = $_SESSION['role_id'];
        $dropcompany = getAjaxCompanyMain($current_role);
        $dropproject = getAjaxProjectMain();
        $hsecatOptn['where'] = [
        ...
        $getAllhsecat = $this->common_model->getAlldata(INSP_HOTEL_CAT, ['*'], $hsecatOptn);
        ...
        $getItemdatas = '';
        if ($did != '') {
            $getUseeActdatas = $this->common_model->getAlldata(INSP_HOTEL_FLOW_SEE, ['*'], $insOptn);
            $getItemdatas = $this->common_model->getAlldata(INSP_HOTEL_ITEMS, ['*'], $itemOptn);
        }

        $data = array(
            'view_file' => 'inspection/hotel/add_form',
            'current_menu' => 'Add Inspection',
            'getProgramdatas' => $getUseeActdatas,
            'getItemdatas' => $getItemdatas,
            'dropcompany' => $dropcompany,
            'dropproject' => $dropproject,
            'drophotelstatus' => $hotel_status,
    ...
    public function view_form($id = '')
    {
        ...
        $insOptn = [
            ...
            'join' => [
                INSP_HOTEL_CAT . ' as category' => [
                    INSP_HOTEL_FLOW_SEE . '.ins_cat_id =  category.id',
                    'LEFT',
                ],
                LOGIN . ' as LD' => [
                    INSP_HOTEL_FLOW_SEE . '.ins_reporter_id =  LD.LOGIN_ID',
                    'LEFT',
                ],
                EMPL . ' as Reporter' => [
        ...
        $itemOptn = [
            ...
            'join' => [
                INSP_HOTEL_SUBCATDATA . ' as subcategorydata' => [
                    INSP_HOTEL_ITEMS . '.fk_item_subcatdata_id = subcategorydata.id',
                    'LEFT',
                ],
                INSP_HOTEL_SUBCAT . ' as subcategory' => [
                    INSP_HOTEL_ITEMS . '.fk_item_subcat_id = subcategory.id',
                    'LEFT',
                ],
                INSP_HOTEL_CAT . ' as category' => [
                    INSP_HOTEL_ITEMS . '.fk_item_cat_id =  category.id',
                    'LEFT',
                ],
            ],
        ];
        ...
        $getEvidencedatas = '';
        if ($did != '') {
            $getUseeActdatas = $this->common_model->getAlldata(INSP_HOTEL_FLOW_SEE, [
                INSP_HOTEL_FLOW_SEE . '.*',
            ...
            $getItemdatas = $this->common_model->getAlldata(INSP_HOTEL_ITEMS, [
                INSP_HOTEL_ITEMS . '.*',
                'subcategorydata.*',
                'subcategory.subcategory',
            ], $itemOptn);

            $getEvidencedatas = $this->common_model->getAlldata(INSP_HOTEL_IMAGE_SEE, ['*'], $imageOptn);

            $data = [
                'view_file' => 'inspection/hotel/view_form',
                'current_menu' => 'View Inspection',
                'formTitle' => 'View Hotel Inspection',
    ...
    public function pdf($id = '')
    {
        ...
        if ($did != '') {
            ...
            $data = [
                'title' => 'Hotel Inspection Details',
                'inspData' => $getUseeActdatas,
                'itemData' => $getItemdatas,
                'evidenceData' => $getEvidencedatas,
            ];
            ...
            $html = $this->load->view('inspection/hotel/list_pdf', $data, true);
            $mpdf->WriteHTML($html);

            $filename = "hotel_ins_details.pdf";
    ...
    public function fetchCategoryDetails()
    {
        ...
        if (!empty($insId)) {
            ...
            $groupedData['evidence_images'] = $this->db->get(INSP_HOTEL_IMAGE_SEE)->result_array();
        }
        ...
        $subcategoryData = $this->db->select('subcatdata.fk_subcat_id, subcatdata.id, subcatdata.subcategorydata, ')
            ->join(INSP_HOTEL_SUBCAT . ' AS subcat', 'subcat.id = subcatdata.fk_subcat_id')
        ...
        $subcategoryData = $this->db->get(INSP_HOTEL_SUBCATDATA . ' AS subcatdata')->result_array();
    ...
    public function save_inspection($id = '')
    {
        ...
        $files = $_FILES['images'];
        $uploadPath = 'assets/images/modules/inspection/hotel/evidence';
        ...
        if ($this->form_validation->run()) {
            ...
            if ($did != '') {
                ...
                $updtProfile = $this->common_model->updateData(INSP_HOTEL_FLOW_SEE, $prodatas, $upWhere);
            } else {
                ...
                $getuniqueId = getHotelInsNumber($projNameThreeLetters, $areaShortTwoLetters, $build_block_dirFirstLetter, postData($InsDatas, 'ins_project_id'));

                $prodatas['ins_id'] = $getuniqueId;
                $prodatas['ins_report_datetime'] = date('Y-m-d H:i:s', strtotime($date_time));
                $updtProfile = $this->common_model->updateData(INSP_HOTEL_FLOW_SEE, $prodatas);
            }
            ...
            $getAutoIddata = $this->common_model->getAlldata(INSP_HOTEL_FLOW_SEE, ["ins_auto_id"], $optionMainId);
            ...
            foreach ($subCatDataIds as $key => $subCatDataId) {
                ...
                $building_block_dirName = getBuildingName(postData($InsDatas, 'ins_building_id'));
                $build_block_dirFirstLetter = strtoupper(substr($building_block_dirName, 0, 1));
                $getuniqueIdAtar = getHotelInsAtarNumber($projNameThreeLetters, $areaShortTwoLetters, $build_block_dirFirstLetter, postData($InsDatas, 'ins_project_id'));
                ...
                if (!empty($ins_item_id[$key])) {
                    ...
                    $insertedItem = $this->common_model->updateData(INSP_HOTEL_ITEMS, $item, $upwhere);
                    ...
                    if ($selections[$key] == '0' && !empty($notes[$key])) {
                        ...
                        $insertedItem = $this->common_model->updateData(INSP_HOTEL_ITEMS, $item);
            ...
            foreach ($uploadedData as $subCatDataIds => $images) {
                foreach ($images as $key => $imageData) {
                    if (!empty($imageData['image_id']) && !empty($imageData['file_path'])) {
                        ...
                        $existingImageFullPath = FCPATH . 'assets/images/modules/inspection/hotel/evidence/' . $existingImageName;
                    ...
                    $insertedItem = $this->common_model->updateData(INSP_HOTEL_IMAGE_SEE, $image, $imgwhere);
                }
            }
            ...
            if ($updtProfile) {
                ...
                if ($isatar) {
                    redirect('inspection/hotel/hotel_atar_list');
                } else {
                    redirect('inspection/hotel/inspection_list');
                }
            } else {
                if ($did != '') {
    ...
    public function delete_inspection()
    {
        ...
        $updtDelete = $this->common_model->updateData(INSP_HOTEL_FLOW_SEE, ['ins_status' => 'N'], ['ins_auto_id' => decryptval($deleteId)]);
        ...
        if ($updtDelete) {
            ...
            $retData = [
                'status' => true,
                'msgs' => 'Hotel Inspection has been deleted successfully'
            ];
        } else {
            ...
            $retData = [
                'status' => false,
                'msgs' => 'Error in deleting Hotel Inspection! Try Again later.'
            ];
        }
    ...
    public function delete_evidence()
    {
        ...
        $updtDelete = $this->common_model->updateData(INSP_HOTEL_IMAGE_SEE, ['insp_attach_status' => 'N'], ['insp_att_id' => $deleteId]);
    ...
    public function Hotel_exportpdf()
    {
        ...
        $data = [
            'header' => $header,
            'content' => $listUsee,
            'pagetitle' => "Hotel Inspection List",
        ];
        ...
        $html = $this->load->view('inspection/hotel/export_pdf', $data, true);
        $mpdf->WriteHTML($html);
    ...
    public function Hotel_exportexcel()
    {
        global $insPermission, $risk_rating_batch, $ins_type_list, $status_main, $insp_status;
        $getdashdata = $this->input->get();
        ...
        SimpleExcelWriter::streamDownload('Hotel Form List.xlsx')
            ->addHeader($header)
            ->addRows($exportData);
    }
    ...
    // ATAR Start
    public function hotel_atar_list()
    {
        $getdashdata = $this->input->get();
        $current_role = $_SESSION['role_id'];
        $dropcompany = getAjaxCompanyMain($current_role);
        $dropproject = getAjaxProjectMain();
        $user_clid = $_SESSION['emp_details']->EMP_COMP_ID;
        global $status_drop, $status_main, $hotel_status;
        ...
        $data = [
            'pageTitle' => 'Hotel Inspection Managemnet',
            'view_file' => 'inspection/hotel/list_atar_form',
            'site_title' => 'Hotel Inspection Managemnet',
            'current_menu' => 'ATAR',
            'dropcompany' => $dropcompany,
            'dropproject' => $dropproject,
            'drophsecat' => $dropinscat,
            ...
            'status_main' => $status_main,
            'getdashdata' => $getdashdata,
            'ajaxurl' => 'inspection/hotel/listAtarInspection?Status=' . $Status . '&company_id=' . $company_id . '&area_id=' . $area_id . '&building_id=' . $building_id  . '&cat_id=' . $cat_id . '&project_id='  . $project_id . '&Department=' . $department_id . '&Start_Date=' . $Start_Date . '&End_Date=' . $End_Date,
        ];
        $this->template->load_table_exp_template($data);
    }
    ...
    {
        global $insPermission, $risk_rating_batch, $ins_type_list, $status_main, $hotel_status;
        $getdashdata = $this->input->get();
        ...
        $options['join'][INSP_HOTEL_CAT . ' as cat'] = ['cat.id = h.fk_item_cat_id', 'left'];
        $options['join'][INSP_HOTEL_FLOW_SEE . ' as inspection'] = ['inspection.ins_auto_id = h.fk_insp_main_auto_id', 'left'];
        ...
        if (isset($listUsee) && !empty($listUsee)) {
            foreach ($listUsee as $ltKey => $ltVal) {
                ...
                if (((in_array($user_type, $insPermission['view_supadmin'])) || (in_array($user_type, $insPermission['view_ad']) || $userid == $ins_reporter_id)) && ($stat == 1 || $stat == 0)) {
                    ...
                    // $action .= " " . anchor('inspection/hotel/add_atar/' . $mid . '/' . $id, '<i class="fa fa-edit"></i>', array('class' => '', 'title' => 'Edit'));
                }
                if (in_array($user_type, $insPermission['view_supadmin']) || (in_array($user_type, $insPermission['view_ad']))) {
                ...
                $action .= " " . anchor('inspection/hotel/view/' . $id, '<i class="fa fa-eye"></i>', array('title' => 'view'));
                // }
                // $action .= " " . anchor('inspection/hotel/pdf/' . $id, '<i class="fas fa-file-pdf" aria-hidden="true"></i>', array('class' => '', 'title' => 'PDF', 'target' => '_blank'));
                ...
                $rows[] = $hotel_status[$ltVal->insp_item_condition];
    ...
    public function Hotel_atar_exportpdf()
    {
        ...
        $data = [
            'header' => $header,
            'content' => $listUsee,
            'pagetitle' => "Hotel Inspection ATAR List",
        ];
        ...
        // Load the view for the PDF content
        $html = $this->load->view('inspection/hotel/atar_export_pdf', $data, true);
        $mpdf->WriteHTML($html);
    ...
    public function Hotel_atar_exportexcel()
    {

        global $insPermission, $risk_rating_batch, $ins_type_list, $status_main, $hotel_status, $status_drop;
        $getdashdata = $this->input->get();
        ...
        if (!empty($listUsee)) {
            foreach ($listUsee as $ltVal) {
                ...
                $row[] = $ltVal->proj_name ?? '-';
                $row[] = $ltVal->ins_cat_name ?? '-';
                $row[] = $hotel_status[$ltVal->insp_item_condition] ?? '-';
        ...
        // Export data to Excel
        SimpleExcelWriter::streamDownload('Hotel ATAR Form List.xlsx')
            ->addHeader($header)
            ->addRows($exportData);
    }
    ...
    public function delete_atar()
    {
        ...
        $updtDelete = $this->common_model->updateData(INSP_HOTEL_ITEMS, ['insp_status' => 'N'], ['insp_item_auto_id' => decryptval($deleteId)]);

        if ($updtDelete) {

            $retData = [
                'status' => true,
                'msgs' => 'Hotel Inspection Atar has been deleted successfully'
            ];
        } else {
            ...
            $retData = [
                'status' => false,
                'msgs' => 'Error in deleting Hotel Inspection Atar! Try Again later.'
            ];
        }
        echo json_encode($retData);
        exit();
    ...
    public function view($id = '')
    {
        ...
        $getItemdatas = '';
        if ($did != '') {
            $editData = $this->hotel->getAtarDetails_project(['insp_item_auto_id' => $did], 'row');
            $getbeforeimage = $this->hotel->getInspImagBefore_project($did);
            $getuafterallincident = $this->hotel->getInspactImageAfter_project($did);
            $getInspAssign = $this->hotel->getInspDataassign_project($did);
            $getInspReassign = $this->hotel->getInspDataReassign_project($did);
            $getInspApproval = $this->hotel->getInspDataapproval_project($did);
            $getInspApprovalFinal = $this->hotel->getInspDataapprovalfinal_project($did);
            $getInspActionTaken = $this->hotel->getInspDataactiontaken_project($did);
        }
        // echo '<pre>';print_r($getInspAssign);die;

        $data = array(
            'view_file' => 'inspection/hotel/atar_view_form',
            'current_menu' => 'View Inspection',
            'editData' => $editData,
    ...
    public function saveStatus($id = '')
    {
        ...
        $optWhr['return_type'] = 'row';
        $getStatdatascronsent = $this->common_model->getAlldata(INSP_HOTEL_ITEMS, ['*'], $optWhrcnt);
        ...
        if ($did != '') {
            ...
            $updtProfile = $this->common_model->updateData(INSP_HOTEL_ITEMS, $useedatas, $upWhere);
            $updttProfile = $this->common_model->updateData(INSP_HOTEL_FLOW_SEE, $useedatas2, $upWhere2);
        }
        ...
        if ($updtProfile) {
            ...
            $this->common_model->updateData(INSP_HOTEL_RISK_FLOW, $risk_log);
            ...
            $optWhr['return_type'] = 'row';
            $getStatdatas = $this->common_model->getAlldata(INSP_HOTEL_ITEMS, ['*'], $optWhr);
            $StatId = $getStatdatas->insp_app_status;
            if ($StatId == 4) {
                sendHotelInspNotification($did);
            }
            ////
            if ($did != '') {
                $this->session->set_flashdata('inspectionflash', '<div class="alert alert-success"><button type="button" class="close" data-dismiss="alert" aria-hidden="true">x</button><span>Success!</span> Inspection has been Updated</div>');
    ...
    public function saveCADetails($id = '')
    {
        ...
        if ($this->form_validation->run()) {
            ...
            $ca_log_id = $this->common_model->updateData(INSP_HOTEL_ACTION_LOG, $useeCAlog);
            ...
            if ($imgUActDatasImage != FALSE && count($imgUActDatasImage) > 0) {
                foreach ($imgUActDatasImage as $fileAct) {
                    ...
                    $this->common_model->updateData(INSP_HOTEL_IMAGE_SEE, $updateUploadActData);
                }
            }
            if ($did != '') {
                $upWhere = [
                ...
                $updtProfile = $this->common_model->updateData(INSP_HOTEL_ITEMS, $useeCA, $upWhere);
            } else {
                $updtProfile = $this->common_model->updateData(INSP_HOTEL_ITEMS, $useeCA);
            }
    ...
    public function saveApproveDetails($id = '')
    {
        ...
        if ($this->form_validation->run()) {
            ...
            if ($did != '') {
                ...
                $updtProfile = $this->common_model->updateData(INSP_HOTEL_ITEMS, $useeApprove, $upWhere);
                $updtProfiles = $this->common_model->updateData(INSP_HOTEL_APP_ESL_LOG, $useeApproveNew);
            }
