...
class Monthly extends CI_Controller
{
    ...
    public function __construct()
    {
        parent::__construct();
        isLogin();
        $this->load->model('inspection/monthly_model', 'monthly');
    ...
    public function inspection_list()
    {
        ...
        $getAllinscat = $this->common_model->getAlldata(INSP_MONTH_CAT, ['*'], $inscatOptn);
        $dropinscat = customFormDropDown($getAllinscat, 'id', 'category', 'Select Category');



        $data = [
            'pageTitle' => 'Monthly Inspection',
            'view_file' => 'inspection/monthly/list_form',
            'site_title' => 'Monthly Inspection',
            'current_menu' => 'List',
            ...
            'ajaxurl' => 'inspection/monthly/listInspection?Status=' . $Status . '&company_id=' . $company_id . '&area_id=' . $area_id . '&building_id=' . $building_id  . '&cat_id=' . $cat_id . '&project_id='  . $project_id . '&Department=' . $department_id . '&Start_Date=' . $Start_Date . '&End_Date=' . $End_Date,
        ];
    ...
    public function listInspection()
    {
        ...
        $table = INSP_MONTH_FLOW_SEE . ' as h';
        $column_order = array(
            null,
            'ins_id',
        ...
        $options['join'][INSP_MONTH_CAT . ' as cat'] = ['cat.id = h.ins_cat_id', 'left'];
        ...
        if (isset($listUsee) && !empty($listUsee)) {
            foreach ($listUsee as $ltKey => $ltVal) {
                ...
                if (((in_array($user_type, $insPermission['view_supadmin'])) || (in_array($user_type, $insPermission['view_ad']) || $userid == $ins_reporter_id)) && ($stat == 1 || $stat == 0) && ($is_edit == 0)) {
                    $action .= " " . anchor('inspection/monthly/add_inspection/' . $id, '<i class="fa fa-edit"></i>', array('class' => '', 'title' => 'Edit'));
                }
                ...
                $action .= " " . anchor('inspection/monthly/view_form/' . $id, '<i class="fa fa-eye"></i>', array('title' => 'view'));
    ...
    public function add_inspection($id = '')
    {
        ...
        global $owner_list, $owner_engineer_list, $EPC_list, $monthly_status;
        $current_role = $_SESSION['role_id'];
        $dropcompany = getAjaxCompanyMain($current_role);
        $dropproject = getAjaxProjectMain();
        $hsecatOptn['where'] = [
        ...
        $getAllhsecat = $this->common_model->getAlldata(INSP_MONTH_CAT, ['*'], $hsecatOptn);
        ...
        $getItemdatas = '';
        if ($did != '') {
            $getUseeActdatas = $this->common_model->getAlldata(INSP_MONTH_FLOW_SEE, ['*'], $insOptn);
            $getItemdatas = $this->common_model->getAlldata(INSP_MONTH_ITEMS, ['*'], $itemOptn);
        }

        $data = array(
            'view_file' => 'inspection/monthly/add_form',
            'current_menu' => 'Add Inspection',
            'getProgramdatas' => $getUseeActdatas,
            'getItemdatas' => $getItemdatas,
            'dropcompany' => $dropcompany,
            'dropproject' => $dropproject,
            'dropmonthlystatus' => $monthly_status,
    ...
    public function view_form($id = '')
    {
        ...
        $insOptn = [
            ...
            'join' => [
                INSP_MONTH_CAT . ' as category' => [
                    INSP_MONTH_FLOW_SEE . '.ins_cat_id =  category.id',
                    'LEFT',
                ],
                LOGIN . ' as LD' => [
                    INSP_MONTH_FLOW_SEE . '.ins_reporter_id =  LD.LOGIN_ID',
                    'LEFT',
                ],
                EMPL . ' as Reporter' => [
        ...
        $itemOptn = [
            ...
            'join' => [
                INSP_MONTH_SUBCATDATA . ' as subcategorydata' => [
                    INSP_MONTH_ITEMS . '.fk_item_subcatdata_id = subcategorydata.id',
                    'LEFT',
                ],
                INSP_MONTH_SUBCAT . ' as subcategory' => [
                    INSP_MONTH_ITEMS . '.fk_item_subcat_id = subcategory.id',
                    'LEFT',
                ],
                INSP_MONTH_CAT . ' as category' => [
                    INSP_MONTH_ITEMS . '.fk_item_cat_id =  category.id',
                    'LEFT',
                ],
            ],
        ];
        ...
        $getEvidencedatas = '';
        if ($did != '') {
            $getUseeActdatas = $this->common_model->getAlldata(INSP_MONTH_FLOW_SEE, [
                INSP_MONTH_FLOW_SEE . '.*',
            ...
            $getItemdatas = $this->common_model->getAlldata(INSP_MONTH_ITEMS, [
                INSP_MONTH_ITEMS . '.*',
                'subcategorydata.*',
                'subcategory.subcategory',
            ], $itemOptn);

            $getEvidencedatas = $this->common_model->getAlldata(INSP_MONTH_IMAGE_SEE, ['*'], $imageOptn);

            $data = [
                'view_file' => 'inspection/monthly/view_form',
                'current_menu' => 'View Inspection',
                'formTitle' => 'View Monthly Inspection',
    ...
    public function pdf($id = '')
    {
        ...
        if ($did != '') {
            ...
            $data = [
                'title' => 'Monthly Inspection Details',
                'inspData' => $getUseeActdatas,
                'itemData' => $getItemdatas,
                'evidenceData' => $getEvidencedatas,
            ];
            ...
            $html = $this->load->view('inspection/monthly/list_pdf', $data, true);
            $mpdf->WriteHTML($html);

            $filename = "monthly_ins_details.pdf";
    ...
    public function fetchCategoryDetails()
    {
        ...
        if (!empty($insId)) {
            ...
            $groupedData['evidence_images'] = $this->db->get(INSP_MONTH_IMAGE_SEE)->result_array();
        }
        ...
        $subcategoryData = $this->db->select('subcatdata.fk_subcat_id, subcatdata.id, subcatdata.subcategorydata, ')
            ->join(INSP_MONTH_SUBCAT . ' AS subcat', 'subcat.id = subcatdata.fk_subcat_id')
        ...
        $subcategoryData = $this->db->get(INSP_MONTH_SUBCATDATA . ' AS subcatdata')->result_array();
    ...
    public function save_inspection($id = '')
    {
        ...
        $files = $_FILES['images'];
        $uploadPath = 'assets/images/modules/inspection/monthly/evidence';
        ...
        if ($this->form_validation->run()) {
            ...
            if ($did != '') {
                ...
                $updtProfile = $this->common_model->updateData(INSP_MONTH_FLOW_SEE, $prodatas, $upWhere);
            } else {
                ...
                $getuniqueId = getMonthlyInsNumber($projNameThreeLetters, $areaShortTwoLetters, $build_block_dirFirstLetter, postData($InsDatas, 'ins_project_id'));

                $prodatas['ins_id'] = $getuniqueId;
                $prodatas['ins_report_datetime'] = date('Y-m-d H:i:s', strtotime($date_time));
                $updtProfile = $this->common_model->updateData(INSP_MONTH_FLOW_SEE, $prodatas);
            }
            ...
            $getAutoIddata = $this->common_model->getAlldata(INSP_MONTH_FLOW_SEE, ["ins_auto_id"], $optionMainId);
            ...
            foreach ($subCatDataIds as $key => $subCatDataId) {
                ...
                $building_block_dirName = getBuildingName(postData($InsDatas, 'ins_building_id'));
                $build_block_dirFirstLetter = strtoupper(substr($building_block_dirName, 0, 1));
                $getuniqueIdAtar = getMonthlyInsAtarNumber($projNameThreeLetters, $areaShortTwoLetters, $build_block_dirFirstLetter, postData($InsDatas, 'ins_project_id'));
                ...
                if (!empty($ins_item_id[$key])) {
                    ...
                    $insertedItem = $this->common_model->updateData(INSP_MONTH_ITEMS, $item, $upwhere);
                    ...
                    if ($selections[$key] == '0' && !empty($notes[$key])) {
                        ...
                        $insertedItem = $this->common_model->updateData(INSP_MONTH_ITEMS, $item);
            ...
            foreach ($uploadedData as $subCatDataIds => $images) {
                foreach ($images as $key => $imageData) {
                    if (!empty($imageData['image_id']) && !empty($imageData['file_path'])) {
                        ...
                        $existingImageFullPath = FCPATH . 'assets/images/modules/inspection/monthly/evidence/' . $existingImageName;
                    ...
                    $insertedItem = $this->common_model->updateData(INSP_MONTH_IMAGE_SEE, $image, $imgwhere);
                }
            }
            ...
            if ($updtProfile) {
                ...
                if ($isatar) {
                    redirect('inspection/monthly/monthly_atar_list');
                } else {
                    redirect('inspection/monthly/inspection_list');
                }
            } else {
                if ($did != '') {
    ...
    public function delete_inspection()
    {
        ...
        $updtDelete = $this->common_model->updateData(INSP_MONTH_FLOW_SEE, ['ins_status' => 'N'], ['ins_auto_id' => decryptval($deleteId)]);
        ...
        if ($updtDelete) {
            ...
            $retData = [
                'status' => true,
                'msgs' => 'Monthly Inspection has been deleted successfully'
            ];
        } else {
            ...
            $retData = [
                'status' => false,
                'msgs' => 'Error in deleting Monthly Inspection! Try Again later.'
            ];
        }
    ...
    public function delete_evidence()
    {
        ...
        $updtDelete = $this->common_model->updateData(INSP_MONTH_IMAGE_SEE, ['insp_attach_status' => 'N'], ['insp_att_id' => $deleteId]);
    ...
    public function Monthly_exportpdf()
    {
        ...
        $data = [
            'header' => $header,
            'content' => $listUsee,
            'pagetitle' => "Monthly Inspection List",
        ];
        ...
        $html = $this->load->view('inspection/monthly/export_pdf', $data, true);
        $mpdf->WriteHTML($html);

        // Output the PDF
        $filename = "Monthly_list.pdf";
        $mpdf->Output($filename, 'D');
    }

    public function Monthly_exportexcel()
    {
        global $insPermission, $risk_rating_batch, $ins_type_list, $status_main, $insp_status;
        $getdashdata = $this->input->get();
        ...
        SimpleExcelWriter::streamDownload('Monthly Form List.xlsx')
            ->addHeader($header)
            ->addRows($exportData);
    }
    ...
    // ATAR Start
    public function monthly_atar_list()
    {
        $getdashdata = $this->input->get();
        $current_role = $_SESSION['role_id'];
        $dropcompany = getAjaxCompanyMain($current_role);
        $dropproject = getAjaxProjectMain();
        $user_clid = $_SESSION['emp_details']->EMP_COMP_ID;
        global $status_drop, $status_main, $monthly_status;
        ...
        $data = [
            'pageTitle' => 'Monthly Inspection Managemnet',
            'view_file' => 'inspection/monthly/list_atar_form',
            'site_title' => 'Monthly Inspection Managemnet',
            'current_menu' => 'ATAR',
            'dropcompany' => $dropcompany,
            'dropproject' => $dropproject,
            'drophsecat' => $dropinscat,
            ...
            'status_main' => $status_main,
            'getdashdata' => $getdashdata,
            'ajaxurl' => 'inspection/monthly/listAtarInspection?Status=' . $Status . '&company_id=' . $company_id . '&area_id=' . $area_id . '&building_id=' . $building_id  . '&cat_id=' . $cat_id . '&project_id='  . $project_id . '&Department=' . $department_id . '&Start_Date=' . $Start_Date . '&End_Date=' . $End_Date,
        ];
        $this->template->load_table_exp_template($data);
    }
    ...
    {
        global $insPermission, $risk_rating_batch, $ins_type_list, $status_main, $monthly_status;
        $getdashdata = $this->input->get();
        ...
        $options['join'][INSP_MONTH_CAT . ' as cat'] = ['cat.id = h.fk_item_cat_id', 'left'];
        $options['join'][INSP_MONTH_FLOW_SEE . ' as inspection'] = ['inspection.ins_auto_id = h.fk_insp_main_auto_id', 'left'];
        ...
        if (isset($listUsee) && !empty($listUsee)) {
            foreach ($listUsee as $ltKey => $ltVal) {
                ...
                if (((in_array($user_type, $insPermission['view_supadmin'])) || (in_array($user_type, $insPermission['view_ad']) || $userid == $ins_reporter_id)) && ($stat == 1 || $stat == 0)) {
                    ...
                    // $action .= " " . anchor('inspection/monthly/add_atar/' . $mid . '/' . $id, '<i class="fa fa-edit"></i>', array('class' => '', 'title' => 'Edit'));
                }
                if (in_array($user_type, $insPermission['view_supadmin']) || (in_array($user_type, $insPermission['view_ad']))) {
                ...
                $action .= " " . anchor('inspection/monthly/view/' . $id, '<i class="fa fa-eye"></i>', array('title' => 'view'));
                // }
                // $action .= " " . anchor('inspection/monthly/pdf/' . $id, '<i class="fas fa-file-pdf" aria-hidden="true"></i>', array('class' => '', 'title' => 'PDF', 'target' => '_blank'));
                ...
                $rows[] = $monthly_status[$ltVal->insp_item_condition];
    ...
    public function Monthly_atar_exportpdf()
    {
        ...
        $data = [
            'header' => $header,
            'content' => $listUsee,
            'pagetitle' => "Monthly Inspection ATAR List",
        ];
        ...
        // Load the view for the PDF content
        $html = $this->load->view('inspection/monthly/atar_export_pdf', $data, true);
        $mpdf->WriteHTML($html);
    ...
    public function Monthly_atar_exportexcel()
    {

        global $insPermission, $risk_rating_batch, $ins_type_list, $status_main, $monthly_status, $status_drop;
        $getdashdata = $this->input->get();
        ...
        if (!empty($listUsee)) {
            foreach ($listUsee as $ltVal) {
                ...
                $row[] = $ltVal->proj_name ?? '-';
                $row[] = $ltVal->ins_cat_name ?? '-';
                $row[] = $monthly_status[$ltVal->insp_item_condition] ?? '-';
        ...
        // Export data to Excel
        SimpleExcelWriter::streamDownload('Monthly ATAR Form List.xlsx')
            ->addHeader($header)
            ->addRows($exportData);
    }
    ...
    public function delete_atar()
    {
        ...
        $updtDelete = $this->common_model->updateData(INSP_MONTH_ITEMS, ['insp_status' => 'N'], ['insp_item_auto_id' => decryptval($deleteId)]);

        if ($updtDelete) {

            $retData = [
                'status' => true,
                'msgs' => 'Monthly Inspection Atar has been deleted successfully'
            ];
        } else {
            ...
            $retData = [
                'status' => false,
                'msgs' => 'Error in deleting Monthly Inspection Atar! Try Again later.'
            ];
        }
        echo json_encode($retData);
        exit();
    ...
    public function view($id = '')
    {
        ...
        $getItemdatas = '';
        if ($did != '') {
            $editData = $this->monthly->getAtarDetails_project(['insp_item_auto_id' => $did], 'row');
            $getbeforeimage = $this->monthly->getInspImagBefore_project($did);
            $getuafterallincident = $this->monthly->getInspactImageAfter_project($did);
            $getInspAssign = $this->monthly->getInspDataassign_project($did);
            $getInspReassign = $this->monthly->getInspDataReassign_project($did);
            $getInspApproval = $this->monthly->getInspDataapproval_project($did);
            $getInspApprovalFinal = $this->monthly->getInspDataapprovalfinal_project($did);
            $getInspActionTaken = $this->monthly->getInspDataactiontaken_project($did);
        }
        // echo '<pre>';print_r($getInspAssign);die;

        $data = array(
            'view_file' => 'inspection/monthly/atar_view_form',
            'current_menu' => 'View Inspection',
            'editData' => $editData,
    ...
    public function saveStatus($id = '')
    {
        ...
        $optWhr['return_type'] = 'row';
        $getStatdatascronsent = $this->common_model->getAlldata(INSP_MONTH_ITEMS, ['*'], $optWhrcnt);
        ...
        if ($did != '') {
            ...
            $updtProfile = $this->common_model->updateData(INSP_MONTH_ITEMS, $useedatas, $upWhere);
            $updttProfile = $this->common_model->updateData(INSP_MONTH_FLOW_SEE, $useedatas2, $upWhere2);
        }
        ...
        if ($updtProfile) {
            ...
            $this->common_model->updateData(INSP_MONTH_RISK_FLOW, $risk_log);
            ...
            $optWhr['return_type'] = 'row';
            $getStatdatas = $this->common_model->getAlldata(INSP_MONTH_ITEMS, ['*'], $optWhr);
            $StatId = $getStatdatas->insp_app_status;
            if ($StatId == 4) {
                sendMonthlyInspNotification($did);
            }
            ////
            if ($did != '') {
                $this->session->set_flashdata('inspectionflash', '<div class="alert alert-success"><button type="button" class="close" data-dismiss="alert" aria-hidden="true">x</button><span>Success!</span> Inspection has been Updated</div>');
    ...
    public function saveCADetails($id = '')
    {
        ...
        if ($this->form_validation->run()) {
            ...
            $ca_log_id = $this->common_model->updateData(INSP_MONTH_ACTION_LOG, $useeCAlog);
            ...
            if ($imgUActDatasImage != FALSE && count($imgUActDatasImage) > 0) {
                foreach ($imgUActDatasImage as $fileAct) {
                    ...
                    $this->common_model->updateData(INSP_MONTH_IMAGE_SEE, $updateUploadActData);
                }
            }
            if ($did != '') {
                $upWhere = [
                ...
                $updtProfile = $this->common_model->updateData(INSP_MONTH_ITEMS, $useeCA, $upWhere);
            } else {
                $updtProfile = $this->common_model->updateData(INSP_MONTH_ITEMS, $useeCA);
            }
    ...
    public function saveApproveDetails($id = '')
    {
        ...
        if ($this->form_validation->run()) {
            ...
            if ($did != '') {
                ...
                $updtProfile = $this->common_model->updateData(INSP_MONTH_ITEMS, $useeApprove, $upWhere);
                $updtProfiles = $this->common_model->updateData(INSP_MONTH_APP_ESL_LOG, $useeApproveNew);
            }
