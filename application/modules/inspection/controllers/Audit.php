...
class Audit extends CI_Controller
{
    ...
    public function __construct()
    {
        parent::__construct();
        isLogin();
        $this->load->model('inspection/audit_model', 'audit');
    ...
    public function inspection_list()
    {
        ...
        $getAllinscat = $this->common_model->getAlldata(INSP_AUDIT_CAT, ['*'], $inscatOptn);
        $dropinscat = customFormDropDown($getAllinscat, 'id', 'category', 'Select Category');



        $data = [
            'pageTitle' => 'Audit Inspection',
            'view_file' => 'inspection/audit/list_form',
            'site_title' => 'Audit Inspection',
            'current_menu' => 'List',
            ...
            'ajaxurl' => 'inspection/audit/listInspection?Status=' . $Status . '&company_id=' . $company_id . '&area_id=' . $area_id . '&building_id=' . $building_id  . '&cat_id=' . $cat_id . '&project_id='  . $project_id . '&Department=' . $department_id . '&Start_Date=' . $Start_Date . '&End_Date=' . $End_Date,
        ];
    ...
    public function listInspection()
    {
        ...
        $table = INSP_AUDIT_FLOW_SEE . ' as h';
        $column_order = array(
            null,
            'ins_id',
        ...
        $options['join'][INSP_AUDIT_CAT . ' as cat'] = ['cat.id = h.ins_cat_id', 'left'];
        ...
        if (isset($listUsee) && !empty($listUsee)) {
            foreach ($listUsee as $ltKey => $ltVal) {
                ...
                if (((in_array($user_type, $insPermission['view_supadmin'])) || (in_array($user_type, $insPermission['view_ad']) || $userid == $ins_reporter_id)) && ($stat == 1 || $stat == 0) && ($is_edit == 0)) {
                    $action .= " " . anchor('inspection/audit/add_inspection/' . $id, '<i class="fa fa-edit"></i>', array('class' => '', 'title' => 'Edit'));
                }
                ...
                $action .= " " . anchor('inspection/audit/view_form/' . $id, '<i class="fa fa-eye"></i>', array('title' => 'view'));
                $action .= " " . anchor('inspection/audit/pdf/' . $id, '<i class="fas fa-file-pdf" aria-hidden="true"></i>', array('class' => '', 'title' => 'PDF', 'target' => '_blank'));
                $action .= " " . anchor('inspection/audit/overall_pdf/' . $id, '<i class="fas fa-file-pdf" aria-hidden="true"></i>', array('class' => '', 'title' => 'PDF', 'target' => '_blank'));
    ...
    public function add_inspection($id = '')
    {
        ...
        $getAllhsecat = $this->common_model->getAlldata(INSP_AUDIT_CAT, ['*'], $hsecatOptn);
        ...
        $getItemdatas = '';
        if ($did != '') {
            $getUseeActdatas = $this->common_model->getAlldata(INSP_AUDIT_FLOW_SEE, ['*'], $insOptn);
            $getItemdatas = $this->common_model->getAlldata(INSP_AUDIT_ITEMS, ['*'], $itemOptn);
        }

        $data = array(
            'view_file' => 'inspection/audit/add_form',
            'current_menu' => 'Add Inspection',
            'getProgramdatas' => $getUseeActdatas,
            'getItemdatas' => $getItemdatas,
            'dropcompany' => $dropcompany,
            'dropproject' => $dropproject,
            'dropauditstatus' => $audit_status,
    ...
    public function view_form($id = '')
    {
        ...
        $insOptn = [
            ...
            'join' => [
                INSP_AUDIT_CAT . ' as category' => [
                    INSP_AUDIT_FLOW_SEE . '.ins_cat_id =  category.id',
                    'LEFT',
                ],
                LOGIN . ' as LD' => [
                    INSP_AUDIT_FLOW_SEE . '.ins_reporter_id =  LD.LOGIN_ID',
                    'LEFT',
                ],
                EMPL . ' as Reporter' => [
        ...
        $itemOptn = [
            ...
            'join' => [
                INSP_AUDIT_SUBCATDATA . ' as subcategorydata' => [
                    INSP_AUDIT_ITEMS . '.fk_item_subcatdata_id = subcategorydata.id',
                    'LEFT',
                ],
                INSP_AUDIT_SUBCAT . ' as subcategory' => [
                    INSP_AUDIT_ITEMS . '.fk_item_subcat_id = subcategory.id',
                    'LEFT',
                ],
                INSP_AUDIT_CAT . ' as category' => [
                    INSP_AUDIT_ITEMS . '.fk_item_cat_id =  category.id',
                    'LEFT',
                ],
            ],
        ];
        ...
        $getEvidencedatas = '';
        if ($did != '') {
            $getUseeActdatas = $this->common_model->getAlldata(INSP_AUDIT_FLOW_SEE, [
                INSP_AUDIT_FLOW_SEE . '.*',
            ...
            $getItemdatas = $this->common_model->getAlldata(INSP_AUDIT_ITEMS, [
                INSP_AUDIT_ITEMS . '.*',
                'subcategorydata.*',
                'subcategory.subcategory',
            ], $itemOptn);

            $getEvidencedatas = $this->common_model->getAlldata(INSP_AUDIT_IMAGE_SEE, ['*'], $imageOptn);

            $data = [
                'view_file' => 'inspection/audit/view_form',
                'current_menu' => 'View Inspection',
                'formTitle' => 'View Audit Inspection',
    ...
    public function pdf($id = '')
    {
        ...
        if ($did != '') {
            ...
            $data = [
                'title' => 'Audit Inspection Details',
                'inspData' => $getUseeActdatas,
                'itemData' => $getItemdatas,
                'evidenceData' => $getEvidencedatas,
            ];
            ...
            $html = $this->load->view('inspection/audit/list_pdf', $data, true);
            $mpdf->WriteHTML($html);

            $filename = "audit_ins_details.pdf";
            $mpdf->Output($filename, 'I');
        }
    }


    public function overall_pdf($id = '')
    {
        ...
        $itemOptn['group_by'] = INSP_AUDIT_ITEMS . '.insp_item_auto_id';
        ...
        $atarOptn = [
            ...
            'join' => [ 
                LOGIN . ' as LD' => [
                    INSP_AUDIT_ITEMS . '.insp_supervisor_id =  LD.LOGIN_ID',
                    'LEFT',
                ],
                EMPL . ' as assigner' => [
                    'LD.USER_REF_ID =  assigner.EMP_AUTO_ID',
                    'LEFT',
                ],
                LOGIN . ' as LD2' => [
                    INSP_AUDIT_ITEMS . '.insp_assigner_id =  LD2.LOGIN_ID',
                    'LEFT',
                ],
                EMPL . ' as res_person' => [
                    'LD.USER_REF_ID =  res_person.EMP_AUTO_ID',
                    'LEFT',
                ],              
                INSP_AUDIT_IMAGE_SEE . ' AS insimg' => [ 
                    'insimg.fk_img_item_id = ' . INSP_AUDIT_ITEMS . '.insp_item_auto_id AND insimg.insp_file_type != 1',
                    'LEFT',
                ],
            ],
        ];
        $atarOptn['group_by'] = INSP_AUDIT_ITEMS . '.insp_item_auto_id';
        ...
        if ($did != '') {
            ...
            $getItemdatas = $this->common_model->getAlldata(INSP_AUDIT_ITEMS, [
                ...
                'subcategorydata.*',
                'subcategory.subcategory',
                'FN_INSP_STATUS(insp_app_status) as status_Name',           
          
                
            ], $itemOptn);

            $getAtardatas = $this->common_model->getAlldata(INSP_AUDIT_ITEMS, [
                INSP_AUDIT_ITEMS . '.*',      
                'assigner.EMP_NAME as assigner',
                'res_person.EMP_NAME as res_person',     
                'FN_INSP_STATUS(insp_app_status) as status_Name',            
               'GROUP_CONCAT(CONCAT(insimg.insp_file_path, "", insimg.insp_filename)) as grouped_images',
            ...
            $html = $this->load->view('inspection/audit/overall_pdf', $data, true);
            $mpdf->WriteHTML($html);

            $filename = "overall_audit_ins_details.pdf";
            $mpdf->Output($filename, 'I');
        }
    }
    ...
    public function fetchCategoryDetails()
    {
        ...
        if (!empty($insId)) {
            ...
            $groupedData['evidence_images'] = $this->db->get(INSP_AUDIT_IMAGE_SEE)->result_array();
        }
        ...
        $subcategoryData = $this->db->select('subcatdata.fk_subcat_id, subcatdata.id, subcatdata.subcategorydata, subcatdata.guidance')
            ->join(INSP_AUDIT_SUBCAT . ' AS subcat', 'subcat.id = subcatdata.fk_subcat_id')
        ...
        $subcategoryData = $this->db->get(INSP_AUDIT_SUBCATDATA . ' AS subcatdata')->result_array();
    ...
    public function save_inspection($id = '')
    {
        ...
        $files = $_FILES['images'];
        $uploadPath = 'assets/images/modules/inspection/audit/evidence';
        ...
        if ($this->form_validation->run()) {
            ...
            if ($did != '') {
                ...
                $updtProfile = $this->common_model->updateData(INSP_AUDIT_FLOW_SEE, $prodatas, $upWhere);
            } else {
                ...
                $getuniqueId = getAuditInsNumber($projNameThreeLetters, $areaShortTwoLetters, $build_block_dirFirstLetter, postData($InsDatas, 'ins_project_id'));

                $prodatas['ins_id'] = $getuniqueId;
                $prodatas['ins_report_datetime'] = date('Y-m-d H:i:s', strtotime($date_time));
                $updtProfile = $this->common_model->updateData(INSP_AUDIT_FLOW_SEE, $prodatas);
            }
            ...
            $getAutoIddata = $this->common_model->getAlldata(INSP_AUDIT_FLOW_SEE, ["ins_auto_id"], $optionMainId);
            ...
            foreach ($subCatDataIds as $key => $subCatDataId) {
                ...
                $building_block_dirName = getBuildingName(postData($InsDatas, 'ins_building_id'));
                $build_block_dirFirstLetter = strtoupper(substr($building_block_dirName, 0, 1));
                $getuniqueIdAtar = getAuditInsAtarNumber($projNameThreeLetters, $areaShortTwoLetters, $build_block_dirFirstLetter, postData($InsDatas, 'ins_project_id'));
                ...
                if (!empty($ins_item_id[$key])) {
                    ...
                    $insertedItem = $this->common_model->updateData(INSP_AUDIT_ITEMS, $item, $upwhere);
                    ...
                    if ($selections[$key] == '0' && !empty($notes[$key])) {
                        ...
                        $insertedItem = $this->common_model->updateData(INSP_AUDIT_ITEMS, $item);
            ...
            foreach ($uploadedData as $subCatDataIds => $images) {
                foreach ($images as $key => $imageData) {
                    if (!empty($imageData['image_id']) && !empty($imageData['file_path'])) {
                        ...
                        $existingImageFullPath = FCPATH . 'assets/images/modules/inspection/audit/evidence/' . $existingImageName;
                    ...
                    $insertedItem = $this->common_model->updateData(INSP_AUDIT_IMAGE_SEE, $image, $imgwhere);
                }
            }
            ...
            if ($updtProfile) {
                ...
                if ($isatar) {
                    redirect('inspection/audit/audit_atar_list');
                } else {
                    redirect('inspection/audit/inspection_list');
                }
            } else {
                if ($did != '') {
    ...
    public function delete_inspection()
    {
        ...
        $updtDelete = $this->common_model->updateData(INSP_AUDIT_FLOW_SEE, ['ins_status' => 'N'], ['ins_auto_id' => decryptval($deleteId)]);
        ...
        if ($updtDelete) {
            ...
            $retData = [
                'status' => true,
                'msgs' => 'Audit Inspection has been deleted successfully'
            ];
        } else {
            ...
            $retData = [
                'status' => false,
                'msgs' => 'Error in deleting Audit Inspection! Try Again later.'
            ];
        }
    ...
    public function delete_evidence()
    {
        ...
        $updtDelete = $this->common_model->updateData(INSP_AUDIT_IMAGE_SEE, ['insp_attach_status' => 'N'], ['insp_att_id' => $deleteId]);
    ...
    public function Audit_exportpdf()
    {
        ...
        $data = [
            'header' => $header,
            'content' => $listUsee,
            'pagetitle' => "Audit Inspection List",
        ];
        ...
        $html = $this->load->view('inspection/audit/export_pdf', $data, true);
        $mpdf->WriteHTML($html);

        // Output the PDF
        $filename = "Audit_list.pdf";
        $mpdf->Output($filename, 'D');
    }

    public function Audit_exportexcel()
    {
        global $insPermission, $risk_rating_batch, $ins_type_list, $status_main, $insp_status;
        $getdashdata = $this->input->get();
        ...
        SimpleExcelWriter::streamDownload('Audit Form List.xlsx')
            ->addHeader($header)
            ->addRows($exportData);
    }
    ...
    // ATAR Start
    public function audit_atar_list()
    {
        $getdashdata = $this->input->get();
        $current_role = $_SESSION['role_id'];
        $dropcompany = getAjaxCompanyMain($current_role);
        $dropproject = getAjaxProjectMain();
        $user_clid = $_SESSION['emp_details']->EMP_COMP_ID;
        global $status_drop, $status_main, $audit_status;
        ...
        $data = [
            'pageTitle' => 'Audit Inspection Managemnet',
            'view_file' => 'inspection/audit/list_atar_form',
            'site_title' => 'Audit Inspection Managemnet',
            'current_menu' => 'ATAR',
            'dropcompany' => $dropcompany,
            'dropproject' => $dropproject,
            'drophsecat' => $dropinscat,
            ...
            'status_main' => $status_main,
            'getdashdata' => $getdashdata,
            'ajaxurl' => 'inspection/audit/listAtarInspection?Status=' . $Status . '&company_id=' . $company_id . '&area_id=' . $area_id . '&building_id=' . $building_id  . '&cat_id=' . $cat_id . '&project_id='  . $project_id . '&Department=' . $department_id . '&Start_Date=' . $Start_Date . '&End_Date=' . $End_Date,
        ];
        $this->template->load_table_exp_template($data);
    }
    ...
    {
        global $insPermission, $risk_rating_batch, $ins_type_list, $status_main, $audit_status;
        $getdashdata = $this->input->get();
        ...
        $options['join'][INSP_AUDIT_CAT . ' as cat'] = ['cat.id = h.fk_item_cat_id', 'left'];
        $options['join'][INSP_AUDIT_FLOW_SEE . ' as inspection'] = ['inspection.ins_auto_id = h.fk_insp_main_auto_id', 'left'];
        ...
        if (isset($listUsee) && !empty($listUsee)) {
            foreach ($listUsee as $ltKey => $ltVal) {
                ...
                if (((in_array($user_type, $insPermission['view_supadmin'])) || (in_array($user_type, $insPermission['view_ad']) || $userid == $ins_reporter_id)) && ($stat == 1 || $stat == 0)) {
                    ...
                    // $action .= " " . anchor('inspection/audit/add_atar/' . $mid . '/' . $id, '<i class="fa fa-edit"></i>', array('class' => '', 'title' => 'Edit'));
                }
                if (in_array($user_type, $insPermission['view_supadmin']) || (in_array($user_type, $insPermission['view_ad']))) {
                ...
                $action .= " " . anchor('inspection/audit/view/' . $id, '<i class="fa fa-eye"></i>', array('title' => 'view'));
                // }
                // $action .= " " . anchor('inspection/audit/pdf/' . $id, '<i class="fas fa-file-pdf" aria-hidden="true"></i>', array('class' => '', 'title' => 'PDF', 'target' => '_blank'));
                ...
                $rows[] = $audit_status[$ltVal->insp_item_condition];
    ...
    public function Audit_atar_exportpdf()
    {
        ...
        $data = [
            'header' => $header,
            'content' => $listUsee,
            'pagetitle' => "Audit Inspection ATAR List",
        ];
        ...
        // Load the view for the PDF content
        $html = $this->load->view('inspection/audit/atar_export_pdf', $data, true);
        $mpdf->WriteHTML($html);
    ...
    public function Audit_atar_exportexcel()
    {

        global $insPermission, $risk_rating_batch, $ins_type_list, $status_main, $audit_status, $status_drop;
        $getdashdata = $this->input->get();
        ...
        if (!empty($listUsee)) {
            foreach ($listUsee as $ltVal) {
                ...
                $row[] = $ltVal->proj_name ?? '-';
                $row[] = $ltVal->ins_cat_name ?? '-';
                $row[] = $audit_status[$ltVal->insp_item_condition] ?? '-';
        ...
        // Export data to Excel
        SimpleExcelWriter::streamDownload('Audit ATAR Form List.xlsx')
            ->addHeader($header)
            ->addRows($exportData);
    }
    ...
    public function delete_atar()
    {
        ...
        $updtDelete = $this->common_model->updateData(INSP_AUDIT_ITEMS, ['insp_status' => 'N'], ['insp_item_auto_id' => decryptval($deleteId)]);

        if ($updtDelete) {

            $retData = [
                'status' => true,
                'msgs' => 'Audit Inspection Atar has been deleted successfully'
            ];
        } else {
            ...
            $retData = [
                'status' => false,
                'msgs' => 'Error in deleting Audit Inspection Atar! Try Again later.'
            ];
        }
        echo json_encode($retData);
        exit();
    ...
    public function view($id = '')
    {
        ...
        $getItemdatas = '';
        if ($did != '') {
            $editData = $this->audit->getAtarDetails_project(['insp_item_auto_id' => $did], 'row');
            $getbeforeimage = $this->audit->getInspImagBefore_project($did);
            $getuafterallincident = $this->audit->getInspactImageAfter_project($did);
            $getInspAssign = $this->audit->getInspDataassign_project($did);
            $getInspReassign = $this->audit->getInspDataReassign_project($did);
            $getInspApproval = $this->audit->getInspDataapproval_project($did);
            $getInspApprovalFinal = $this->audit->getInspDataapprovalfinal_project($did);
            $getInspActionTaken = $this->audit->getInspDataactiontaken_project($did);
        }
        // echo '<pre>';print_r($getInspAssign);die;

        $data = array(
            'view_file' => 'inspection/audit/atar_view_form',
            'current_menu' => 'View Inspection',
            'editData' => $editData,
    ...
    public function saveStatus($id = '')
    {
        ...
        $optWhr['return_type'] = 'row';
        $getStatdatascronsent = $this->common_model->getAlldata(INSP_AUDIT_ITEMS, ['*'], $optWhrcnt);
        ...
        if ($did != '') {
            ...
            $updtProfile = $this->common_model->updateData(INSP_AUDIT_ITEMS, $useedatas, $upWhere);
            $updttProfile = $this->common_model->updateData(INSP_AUDIT_FLOW_SEE, $useedatas2, $upWhere2);
        }
        ...
        if ($updtProfile) {
            ...
            $this->common_model->updateData(INSP_AUDIT_RISK_FLOW, $risk_log);
            ...
            $optWhr['return_type'] = 'row';
            $getStatdatas = $this->common_model->getAlldata(INSP_AUDIT_ITEMS, ['*'], $optWhr);
            $StatId = $getStatdatas->insp_app_status;
            if ($StatId == 4) {
                sendAuditInspNotification($did);
            }
            ////
            if ($did != '') {
                $this->session->set_flashdata('inspectionflash', '<div class="alert alert-success"><button type="button" class="close" data-dismiss="alert" aria-hidden="true">x</button><span>Success!</span> Inspection has been Updated</div>');
    ...
    public function saveCADetails($id = '')
    {
        ...
        if ($this->form_validation->run()) {
            ...
            $ca_log_id = $this->common_model->updateData(INSP_AUDIT_ACTION_LOG, $useeCAlog);
            ...
            if ($imgUActDatasImage != FALSE && count($imgUActDatasImage) > 0) {
                foreach ($imgUActDatasImage as $fileAct) {
                    $updateUploadActData = [
                        'fk_insp_main_id' => postData($statusDatas, 'fk_insp_main_auto_id'),
                        'insp_file_type' => '3',
                        'fk_img_item_id' => $did,
                        'fk_insp_cat_id' => postData($statusDatas, 'fk_item_cat_id_at'),
                        'fk_insp_subcat_id' => postData($statusDatas, 'fk_item_subcat_id_at'),
                    ...
                    $this->common_model->updateData(INSP_AUDIT_IMAGE_SEE, $updateUploadActData);
                }
            }
            if ($did != '') {
                $upWhere = [
                ...
                $updtProfile = $this->common_model->updateData(INSP_AUDIT_ITEMS, $useeCA, $upWhere);
            } else {
                $updtProfile = $this->common_model->updateData(INSP_AUDIT_ITEMS, $useeCA);
            }
    ...
    public function saveApproveDetails($id = '')
    {
        ...
        if ($this->form_validation->run()) {
            ...
            if ($did != '') {
                ...
                $updtProfile = $this->common_model->updateData(INSP_AUDIT_ITEMS, $useeApprove, $upWhere);
                $updtProfiles = $this->common_model->updateData(INSP_AUDIT_APP_ESL_LOG, $useeApproveNew);
            }
