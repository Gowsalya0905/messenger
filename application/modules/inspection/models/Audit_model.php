...
class Audit_model extends CI_Model
{
    ...
    public function getInspImagBefore_project($did = '')
    {
        ...
        return $details = $this->common->getAlldata(INSP_AUDIT_IMAGE_SEE, ['*'], $option);
    }
    ...
    public function getAtarDetails_project($where, $type, $option = [])
    {
        ...
        $option['join'] = [
            ...
            // LOGIN . ' AS LD1' => ['LD1.LOGIN_ID = ' . INSP_AUDIT_FLOW_SEE . '.obs_supervisor_id', 'LEFT'],
            // EMPL . ' AS Hsse' => ['Hsse.EMP_AUTO_ID = LD1.USER_REF_ID', 'LEFT'],

            // LOGIN . ' AS LD5' => ['LD5.LOGIN_ID = ' . INSP_AUDIT_FLOW_SEE . '.obs_assigner_id', 'LEFT'],
            // EMPL . ' AS Unithead' => ['Unithead.EMP_AUTO_ID = LD5.USER_REF_ID', 'LEFT'],


            // OBS_RISK_LOG . ' AS RL' => [
            //     "RL.fk_obs_risk_auto_main_id = " . INSP_AUDIT_FLOW_SEE . ".obs_auto_id  AND RL.obs_risk_assigned_type_id = 1",
            //     "LEFT"
            // ],
            ...
            'insp_audit_subcategory_master AS ins_subcate_mas'=>['ins_subcate_mas.id = '.INSP_AUDIT_ITEMS.'.fk_item_subcat_id','LEFT'],
            'insp_audit_subcategorydata_master AS ins_subcatedata_mas'=>['ins_subcatedata_mas.id = '.INSP_AUDIT_ITEMS.'.fk_item_subcatdata_id','LEFT'],
              INSP_AUDIT_FLOW_SEE.' AS insp_main'=>['insp_main.ins_auto_id = '.INSP_AUDIT_ITEMS.'.fk_insp_main_auto_id','LEFT'],

        ];
        $aOption = $option + $option;

        return $details = $this->common->getAlldata(INSP_AUDIT_ITEMS, [
            '*',
            'insp_main.ins_id',
            ...
            'ins_subcatedata_mas.guidance as guidance',
        ], $aOption);
    }
    ...
    public function getInspDataReassign_project($did = "")
    {
        ...
        $option["join"] = [
            LOGIN . " AS LD6" => [
                "LD6.LOGIN_ID = " . INSP_AUDIT_ESL_LOG . ".fk_insp_supervisor_login_id",
                "LEFT",
            ],
            EMPL . " AS repman" => [
            ...
            LOGIN . " AS LD7" => [
                "LD7.LOGIN_ID = " . INSP_AUDIT_ESL_LOG . ".fk_new_assigner_id",
                "LEFT",
            ],
            EMPL . " AS aiemp" => [
    ...
    public function getInspDataapproval_project($did = "")
    {
        ...
        $option["join"] = [
            LOGIN . " AS LD6" => [
                "LD6.LOGIN_ID = " . INSP_AUDIT_APP_ESL_LOG . ".approver_login_id",
                "LEFT",
            ],
            EMPL . " AS repman" => [
                "repman.EMP_AUTO_ID = LD6.USER_REF_ID",
            ...
            LOGIN . " AS LD7" => [
                "LD7.LOGIN_ID = " . INSP_AUDIT_APP_ESL_LOG . ".fk_assigner_login_id",
                "LEFT",
            ],
            EMPL . " AS aiemp" => [
                "aiemp.EMP_AUTO_ID = LD7.USER_REF_ID",
    ...
    public function getInspDataactiontaken_project($did = "")
    {
        ...
        $option["join"] = [
            LOGIN . " AS LD6" => [
                "LD6.LOGIN_ID = " . INSP_AUDIT_ACTION_LOG . ".fk_insp_assigner_login_id",
                "LEFT",
            ],
            EMPL . " AS repman" => [
                "repman.EMP_AUTO_ID = LD6.USER_REF_ID",
            ...
            INSP_AUDIT_IMAGE_SEE . " AS insimg" => [ // Join with obs_image_upload
                "insimg.fk_ca_log_id = " . INSP_AUDIT_ACTION_LOG . ".ca_log_id",
                "LEFT",
            ],
        ];
